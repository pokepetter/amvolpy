...
class Synth:
    def __init__(self, name='default_synth', sample='uoiowa_piano', volume=.5, attack=0, sus=1, falloff=2, chord_shape=(0,-2,2), chord_delays=None, octave_offset=0, pan=-.05, spread=.05):
    ...
    def play(self, note, volume=None, attack=None, sus=0, falloff=None, octave=0, pan=0, spread=.1):
        if not hasattr(self, 'samples_and_pitches'):
            self.samples_and_pitches, attack, falloff, loop_samples = load_instrument(self.sample)
        ...
        note = scale_changer.note_offset(note) + (12*self.octave_offset) + (octave*12)
        sample, pitch = self.samples_and_pitches[note]
        print('-----------play note', volume, attack, sus, falloff)
        a = Audio2(sample, pitch=pitch, volume=0, autoplay=False, balance=pan, spread=spread, loop=True)
        a.play()
        # print('--------------------------------------', attack, sus. falloff)
        a.animate('volume', volume, duration=attack, curve=curve.linear)
        a.animate('volume', 0, delay=attack+sus, duration=falloff, curve=curve.linear)
        destroy(a, delay=(attack+sus+falloff)+3)
...
class AlternatingPiano(Synth):
    def play(self, note, volume=None, attack=None, sus=0, falloff=None, octave=0, pan=0, spread=.1):
        ...
        self.i += 1
        pan = (self.i % 2) -.5
        super().play(note=note, volume=volume, attack=attack, sus=sus, falloff=falloff, octave=octave, pan=pan, spread=spread)

class PannedPiano(Synth):
    def play(self, note, volume=None, attack=None, sus=0, falloff=None, octave=0, pan=0, spread=None):
        # pan based on pitch
        pan = self.pan
        if pan == 0:
            pan = (note/24) -.75
...
class UrsfxSynth(Synth):
    # ursfx default note is A4 (400hz, midi note 69)
    def play(self, note, volume=None, attack=None, sus=None, falloff=None, octave=0, pan=0, spread=None):
        if volume is None: volume = self.volume
        if attack is None: attack = self.attack
        if sus is None: sus = self.sus
        if falloff is None: falloff = self.falloff
        ...
        # note = note - 69
        note = scale_changer.note_offset(note) + (12*self.octave_offset) + (octave*12)
        print('play urfx note:', attack, sus, falloff)
        from ursina.prefabs.ursfx import ursfx
        volume *= lerp(1, .1, note/64)  # lower volume on higher notes
        speed = lerp(3, 6, note/64)  # faster on higher notes
        print(volume)
        ursfx([(0.0, 0.0), (attack, 1.0), (attack+sus, 1.0), (attack+sus+falloff, 0.0), (attack+sus+falloff, 0.0)], volume=volume, wave=self.sample, pitch=note-69, speed=speed)
...
# Audio('square_n45', volume=.2, loop=True)
chip_drum = UrsfxSynth(name='chip_drum', sample='noise', sus=2, falloff=3, octave_offset=2)
chip_square = UrsfxSynth(name='chip_square', sample='square', sus=2, falloff=3, octave_offset=2)
...
rtns = Empty(instrument=panned_piano, volume=.5, chord_delays=0, octave=0)     # real time note section
        ...
        for i, note_section in enumerate(note_sections):
            ...
            for loop in range(note_section.loops):
                ...
                for j, (note, dur) in enumerate(zip(note_section.notes, note_section.durations)):
                    ...
                    delay = note_section.delay+(absolute_time*note_section.time_scale/note_section.speed)
                    if delay < self.T:
                        continue
                    ...
                    if not note_section.is_chord[j]:
                        invoke(note_section.instrument.play, note=note,
                            attack=attack/note_section.time_scale, sus=sus/note_section.time_scale, falloff=falloff/note_section.time_scale,
                            octave=note_section.octave, volume=note_section.volume,
                            # chord_shape=chord_shape, chord_delays=note_section.chord_delays,
                            delay=delay, pan=pan, spread=spread)
                    else:
...
if __name__ == '__main__':
    ...
    melody = NoteSection('[qwe]rty[uio]', octave=-1, speed=8, loops=2, falloff=1/16, chord_delays=1/32, volume=.5, sus=.2, attack=.025)
    melody = NoteSection('[qwe]rty[uio]', octave=1, speed=8, loops=2, falloff=1/16, chord_delays=1/32, volume=.5, sus=.2, attack=.025, delay=1)
